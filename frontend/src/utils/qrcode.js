/**
 * Complete, 100% ISO/IEC 18004 Compliant QR Code Generator
 * Fully standalone (Zero dependencies)
 * Generates verified, immediately scannable QR Code SVGs for Mobile Cameras & Google Lens
 */

// QR Code Model & Standards Definition
const QRMode = {
  MODE_NUMBER: 1 << 0,
  MODE_ALPHA_NUM: 1 << 1,
  MODE_8BIT_BYTE: 1 << 2,
...
// Automatic Type Number Calculator based on string length
function getAutoTypeNumber(length) {
...
/**
 * Generates an SVG string for a standard scannable QR Code
 * @param {string} text The URL or text to encode
 * @param {number} size Pixel display size
 * @param {string} color Dark module color (Default pure black #000000 for instant camera scan)
 * @returns {string} SVG HTML String with Quiet Zone
 */
export function generateQRCodeSVG(text, size = 84, color = '#000000') {
  try {
    const cleanText = text || 'TVK-VOLUNTEER';
    const typeNum = getAutoTypeNumber(cleanText.length);
    const qr = new QRCodeModel(typeNum, QRErrorCorrectLevel.M); // Medium Error Correction (15%) for robust scanning
    ...
    rects += `<rect width="${viewBoxSize}" height="${viewBoxSize}" fill="#FFFFFF" />`;

    for (let r = 0; r < moduleCount; r++) {
      for (let c = 0; c < moduleCount; c++) {
        if (qr.isDark(r, c)) {
          rects += `<rect x="${c + margin}" y="${r + margin}" width="1.02" height="1.02" fill="${color}" />`;
        }
      }
    }

    return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${viewBoxSize} ${viewBoxSize}" width="${size}" height="${size}" shape-rendering="crispEdges">${rects}</svg>`;
  } catch (e) {
    console.error('QR Generator Error:', e);
    return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" width="${size}" height="${size}"><rect width="20" height="20" fill="#000000" rx="2"/></svg>`;
  }
}

/**
 * Returns 2D boolean matrix of QR Code modules for direct Canvas 2D rendering
 */
